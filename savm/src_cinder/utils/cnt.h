...
FORCE_INLINE uint64_t cls64(uint64_t x)
{
  return (uint64_t)__builtin_clrsbll((long long)x);
}
FORCE_INLINE uint32_t cls32(uint32_t x)
{
  return (uint32_t)__builtin_clrsb((int)x);
}
FORCE_INLINE uint16_t cls16(uint16_t x)
{
  // __builtin_clrsb operates on 32-bit int; subtract the 16 upper bits
  return (uint16_t)(__builtin_clrsb((int)(int16_t)x) - 16);
}
FORCE_INLINE uint8_t cls8(uint8_t x)
{
  // __builtin_clrsb operates on 32-bit int; subtract the 24 upper bits
  return (uint8_t)(__builtin_clrsb((int)(int8_t)x) - 24);
}
