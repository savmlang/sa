...
pub mod boxed {
    use std::{
        ffi::c_void, marker::PhantomData, ops::{Deref, DerefMut},
        ptr,
    };
    use crate::FFISafe;
    #[repr(C)]
    ...
    impl RTSafeBoxWrapper {
        pub fn new<T: FFISafe>(data: T) -> RTBox<T> {
            unsafe {
                ...
                if _data.is_null() {
                    {
                        ::core::panicking::panic_fmt(
                            format_args!("Unable to construct"),
                        );
                    };
                }
                ...
                let structdata_ptr = libc::aligned_malloc(
                    size_of::<Self>(),
                    align_of::<Self>(),
                ) as *mut Self;
...
pub mod futures {
    use std::{
        marker::PhantomData, os::raw::c_void, ptr::{self, null_mut},
        task::{Poll, Waker},
    };
    use crate::{FFISafe, boxed::{RTBox, RTSafeBoxWrapper}};
    pub mod implements {
        use std::{
            ffi::c_void, marker::PhantomData, ptr::null_mut, sync::Arc, time::Duration,
        };
        use tokio::{
            spawn, sync::{Mutex, mpsc::{Sender, channel}},
            task::JoinHandle, time,
        };
        use crate::{
            FFISafe, boxed::{RTBox, RTSafeBoxWrapper},
        ...
        pub fn create_future<T: FFISafe + 'static>(
            fut: JoinHandle<RTBox<T>>,
        ) -> FutureTask<T> {
            ...
            spawn(async move {
                ...
                loop {
                    ...
                    if hwnd.is_finished() {
                        lock.finished = true;
                        lock.output = Some(
                            SafeContainer(hwnd.await.expect("Unknown Error").into_raw()),
                        );
                        drop(lock);
                        break;
                    }
            ...
            let state = Box::into_raw(Box::new(StateData { fut: local_state, tx }))
                as *mut c_void;
            FutureTask {
                _output: PhantomData,
                _state: state,
...
pub mod string {
    pub mod str {
        use std::{ops::Deref, ptr, slice, str::{self, Utf8Error}};
        #[repr(C)]
        ...
        impl SharableStr {
            pub fn create(data: &str) -> Self {
                let length = data.len();
                let _raw = unsafe {
                    libc::aligned_malloc(length * size_of::<u8>(), align_of::<u8>())
                } as *mut u8;
                if _raw.is_null() {
                    {
            ...
            pub unsafe fn as_str_unchecked<'a>(data: *const Self) -> &'a str {
                ...
                unsafe {
                    str::from_utf8_unchecked(slice::from_raw_parts(data._raw, data.len))
                }
            }
            /// Please note that the lifetime <'a> refers to the lifetime of the
            /// const reference
        ...
        impl Drop for SharableStr {
            fn drop(&mut self) {
                unsafe {
                    libc::aligned_free(self._raw as _);
                }
            }
        }
    }
...
pub mod vector {
    use core::ffi::c_void;
    use std::{
        num::NonZeroUsize, ops::{Index, IndexMut},
        ptr,
    };
    use crate::FFISafe;
    #[repr(C)]
    ...
    impl<T: FFISafe + Sized> Drop for Vector<T> {
        fn drop(&mut self) {
            unsafe {
                ...
                libc::aligned_free(self.ptr as _)
            };
        }
    }
}
